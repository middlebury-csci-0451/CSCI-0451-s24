...
First, you'll need to join Slack. Just follow the link on the Canvas Syllabus page to join the workspace. You may choose to download the Slack desktop client, but that's up to you. You can make your own choices about notifications, but please do whatever you need to do to ensure that you're able to check Slack on a regular basis. 

# Anaconda Python

An important part of CSCI 0451 is navigating the Python package ecosystem. We will do so using the Anaconda distribution of Python. Getting set up with Anaconda is a somewhat detailed process, outlined below.
...
Open VSCode. [Following these instructions](https://code.visualstudio.com/docs/datascience/jupyter-notebooks), open a Jupyter Notebook in VSCode using the `ml-0451` Anaconda environment as the notebook kernel. 
...
Finally, run the cell. If you get no errors, only the output of the `print` statement, then you did it! 
...
There is a VSCode extension for working with Quarto documents which I highly recommend. Find it [here](https://marketplace.visualstudio.com/items?itemName=quarto.quarto). Here is [a demo](https://quarto.org/docs/get-started/hello/vscode.html) using this extension. 
...
- Tutorials for [JupyterLab](https://www.tutorialspoint.com/jupyter/jupyterlab_interface.htm) if you prefer that over VSCode.  
