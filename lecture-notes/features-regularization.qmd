...
Visually this *should* be pretty easy data to classify. 
...
Let's now run logistic regression with degree-2 polynomial features on this data set. The most convenient way to make this happen in the `scikit-learn` framework is with at `Pipeline`. The `Pipeline` first applies the feature map and then calls the model during both fitting and evaluation. 

```{python}
from sklearn.preprocessing import PolynomialFeatures
from sklearn.pipeline import Pipeline

poly_LR = Pipeline([("poly", PolynomialFeatures(degree = 2)),
                    ("LR", LogisticRegression())])

poly_LR.fit(X, y)

plot_decision_regions(X, y, clf = poly_LR)
score = plt.gca().set_title(f"Training Accuracy = {poly_LR.score(X, y)}")
```

Let's check the coefficients of the model: 

```{python}
poly_LR.named_steps["LR"].coef_.round(2)
```
...
Part of the reason this might be beneficial is that for some data sets, we might not really know what specific features we should try. For example, here's another one where a linear classifier doesn't do so great: 
```{python}
np.random.seed(123)
X, y = make_moons(200, shuffle = True, noise = 0.2)

poly_LR = Pipeline([("poly", PolynomialFeatures(degree = 1)),
                    ("LR", LogisticRegression())])
...
```{python}
poly_LR = Pipeline([("poly", PolynomialFeatures(degree = 5)),
                    ("LR", LogisticRegression())])
...
```{python}
poly_LR = Pipeline([("poly", PolynomialFeatures(degree = 15)),
...
for rep in range(20):
    ...
    for deg in degs:
        poly_LR = Pipeline([("poly", PolynomialFeatures(degree = deg)),
                            ("LR", LogisticRegression(penalty = "none", max_iter = 1e3))])

        poly_LR.fit(X_train, y_train)

        to_add = pd.DataFrame({"deg" : [deg],
                               "train" : [poly_LR.score(X_train, y_train)],
                               "test" : [poly_LR.score(X_test, y_test)]})
...
```{python}
poly_LR = Pipeline([("poly", PolynomialFeatures(degree = 15)),
                    ("LR", LogisticRegression(penalty = "l1", solver = "liblinear", C = .01)), ])
