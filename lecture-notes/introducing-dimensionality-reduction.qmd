...
**Unsupervised learning** refers to machine learning techniques in which we do **not** have access to any targets $\vy$. Instead, we only have the features $\mX$. In general, the aim of unsupervised learning is not to make predictions, but rather to "find some structure" in the $\mX$. Here are some common examples of unsupervised learning: 

## Clustering

Clustering is superficially similar to classification. Suppose we have some data $\mX$ that, when we plot it, looks like this: 
...
fig, ax = plt.subplots(1, figsize = (4, 4))
a = ax.scatter(X[:, 0], X[:, 1], c = clusters)
a = ax.set(xlabel = "Feature 1", ylabel = "Feature 2")
```

We can think of clustering as the task of identifying "similar" or "related" data observations (rows of $\mX$) and grouping them together. 
