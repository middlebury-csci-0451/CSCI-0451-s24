...
```{python}
#| fig-cap: Original data is shown in purple and yellow. Centroids of each cluster are shown in black. 
...
def plot_graph(X, A, z = None, ax = None, show_edge_cuts = True):
    ...
    if z is None:
        nx.draw(G, pos = X, alpha = .4, node_color = "grey", node_size = 100, ax = ax)
    else: 
        if show_edge_cuts:
            colors = ["red" if z[i] != z[j] else "grey" for i, j in G.edges()]
            widths = [5 if z[i] != z[j] else 1 for i, j in G.edges()]
        else:
            colors = "black"
            widths = 1
        
        nx.draw(G, pos = X, alpha = .4, node_color = z, node_size = 100, edge_color = colors, width = widths, ax = ax, cmap=plt.cm.cividis)
...
```{python}
from hidden.spectral import cut
...
```{python}
from hidden.spectral import normcut
