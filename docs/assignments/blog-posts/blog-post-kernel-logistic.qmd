...
- `predict(self, X)` should accept a new feature matrix and return binary labels $\{0,1\}$. For each row of $\mX$, the prediction is obtained using the formula $\mathbb{1}[\bracket{\hat{\v}, \vkappa(\vx)}]$. To do this: 
